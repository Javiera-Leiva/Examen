...
export class Categoria extends Component {
    ...
    refreshList() {
        fetch(variables.API_URL+'ctplato')
    ...
    changeCategoriaNombre = (e) => {
        this.setState({CategoriaP:e.target.value });
    }
    ...
    editClick(cat) {
        this.setState({
            ...
            Id_CategoriaPlato:cat.Id_CategoriaPlato,
            Nombre:cat.Nombre
    ...
    createClick() {
        fetch(variables.API_URL +'ctplato', {
            ...
            body: JSON.stringify({
                CategoriaP:this.state.CategoriaP
            })
        })
            .then(res=>res.json())
            .then((result)=>{
    ...
    updateClick(id) {
        fetch(variables.API_URL +'ctplato/'+id, {
            method:'PUT',
            headers: {
                'Accept':'application/json',
                'Content-Type':'application/json'
            ...
            body: JSON.stringify({
                Id_CategoriaPlato: this.state.Id_CategoriaPlato,
                CategoriaP:this.state.CategoriaP
            })
        })
            .then(res => res.json())
        ...
        return (
            <div>
                ...
                <table className="table table-dark">
                    ...
                    <tbody>
                        {categorias.map(cat=>
                            <tr key={cat.Id_CategoriaPlato}>
                                <td>{cat.Id_CategoriaPlato}</td>
                                <td>{cat.CategoriaP}</td>
                                <td>
                                    <button type="button"
                                    ...
                                    <button type="button"
                                        ...
                                        onClick={() => this.deleteClick(cat.Id_CategoriaPlato)}>
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                {Id_CategoriaPlato !==0?
                                    <button type="button"
                                        className="btn btn-primary float-start"
                                        onClick={() => this.updateClick(Id_CategoriaPlato)}
                                    >Actualizar</button>
                                    : null}
                            </div>
                        </div>
